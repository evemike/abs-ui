...
const Ea = {
  ...
  setup(z, { expose: B, emit: R }) {
    ...
    const K = async (d, p) => {
      ...
      for (const J of d) {
        ...
        })) == null ? void 0 : P.data) == null ? void 0 : ie.data, o++;
      }
      ...
      const o = h(M, e) || [];
      return `select * from ${d.map((v, P) => `${o[P] ? o[P] : ""}${v}${o[P] ? o[P] : ""}`).join(".")}`;
    }, A = S(!1), H = S(!1), fe = async () => {
      H.value = !0;
      try {
        A.value = await (D == null ? void 0 : D(s)) || !1;
      } catch (d) {
  ...
  setup(z, { emit: B }) {
    const R = z, { inputProps: F, outputProps: V, formdata: q, outputFileds: m, inputFileds: U, data: O = [] } = R, $ = B, D = G(() => h(q, U) || []), Y = G(() => h(q, m) || []), L = G(() => {
      ...
      return a(D).map((n) => ({ name: h(n, i.name), type: h(n, i.type), remark: h(n, i.remark) }));
    }), l = S(O), r = () => {
      const i = [];
      for (const n of a(t)) {
    ...
    const s = (i, n) => {
      ...
      if (a(l).length > 0) {
        ...
        $("change", i), $("update:data", a(l));
      }
    };
    return Oa(() => {
      if (O.length == 0 && a(D).length > 0) {
        debugger;
        r();
      }
    }), (i, n) => (f(), I("div", Ga, [
      ...
      N("div", Qa, [
        Ha,
        (f(!0), I(de, null, pe(l.value, (_) => (f(), I("div", Ja, [
          _.$enabled != !1 ? (f(), I("div", Wa)) : (f(), I("div", Za))
        ]))), 256))
      ]),
      N("div", et, [
        x(a(be), {
            ...
            x(a(j), {
              ...
              default: y((_) => [
                x(a(ne), {
                  ...
                  default: y(() => [
                    (f(!0), I(de, null, pe(L.value, (c) => (f(), E(a(ja), {
                      label: c.label,
                      value: c.value
  ...
  const z = "TableOutput", B = {
    ...
    if (T(t, "$tableIds", k), T(t, "$server", s), s && r) {
      r[1] && T(t, "schema", r[1].name), r[2] && T(t, "table", r[2].name);
      const i = _e(s);
      T(t, "connection", s.id), T(t, "$connection", { ...i, name: s.id, dataSourceName: s.name, dsName: s.name, dataSourceId: s.id });
      const _ = await V({
    ...
    if (l && l.length > 0) {
      const r = l.map(([s, k]) => ({ edit: !1, column_name: s.name, stream_name: k.name }));
      T(t, "fields", { field: r });
    }
  };
  ...
  `, rawContent: !0 }, "@change": ({ formData: t, $: l }) => F(t, l[0]), cls: [{ cls: "覆盖", label: "over" }, { cls: "仅插入", label: "insert" }, { cls: "插入且更新", label: "update" }, { cls: "插入不更新", label: "add" }] },
    ...
    { type: "form", label: "高级配置", column: [
      ...
      { label: "裁剪表", tag: C, prop: "truncate", activeValue: "Y", inactiveValue: "N", inlinePrompt: !0, activeText: "Y", inactiveText: "N" },
      { label: "忽略插入错误", tag: C, prop: "ignore_errors", activeValue: "Y", inactiveValue: "N", inlinePrompt: !0, activeText: "Y", inactiveText: "N" },
      { label: "指定数据可字段", tag: C, prop: "specify_fields", activeValue: "Y", inactiveValue: "N", inlinePrompt: !0, activeText: "Y", inactiveText: "N" },
      // {prop:"",label:"",tag:ElSwitch,trueLabel:"Y",falseLabel:"N"}
      { tag: ae, cls: "主选项" },
      { label: "表分区数据", tag: C, prop: "partitioning_enabled", activeValue: "Y", inactiveValue: "N", inlinePrompt: !0, activeText: "Y", inactiveText: "N" },
      { label: "分区字段", tag: ne, prop: "partitioning_field", cls: [] },
      //  item in step.outFields
      { label: "每月分区数据", tag: C, prop: "partitioning_daily", ":disabled": "formData['partitioning_enabled'] == 'N'", activeValue: "Y", inactiveValue: "N", inlinePrompt: !0, activeText: "Y", inactiveText: "N" },
      { label: "每天分区数据", tag: C, prop: "partitioning_monthly", ":disabled": "formData['partitioning_enabled'] == 'N'", activeValue: "Y", inactiveValue: "N", inlinePrompt: !0, activeText: "Y", inactiveText: "N" },
      { label: "使用批量插入", tag: C, prop: "use_batch", ":disabled": "formData['return_keys'] == true", activeValue: "Y", inactiveValue: "N" },
      { label: "表名定义在一个字段里", tag: C, prop: "tablename_in_field", activeValue: "Y", inactiveValue: "N" },
      { label: "包含表名的字段", tag: ne, prop: "tablename_field", clearable: !0, ":disabled": "formData['tablename_in_field'] == 'N'", cls: [] },
      { label: "存储表名字段", tag: C, prop: "tablename_in_table", ":disabled": "formData['tablename_in_field'] == 'N'", activeValue: "Y", inactiveValue: "N", inlinePrompt: !0, activeText: "Y", inactiveText: "N" },
      { label: "返回一个自动产生的关键字", tag: C, prop: "return_keys", activeValue: "Y", inactiveValue: "N", inlinePrompt: !0, activeText: "Y", inactiveText: "N" },
